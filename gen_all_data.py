"""Generate all data for superres experiments."""
import os
import pickle
import numpy as np
...
# Systems to generate
systems = [
    ('single_pendulum', 'data_gen.configs.single_pendulum'),
...
for system_name, config_module in systems:
    ...
    print(f'{system_name} train: {len(trajectories)} trajectories')
    ...
    with open(os.path.join(config.save_dir, 'data.pkl'), 'wb') as fp:
        pickle.dump(trajectories, fp)
    print(f'{system_name} test: {len(trajectories)} trajectories')
